...
namespace IndieStudio.DrawingAndColoring.DCEditor
{
	...
	public class AdsManagerEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.Separator();
			if (PrefabUtility.GetCorrespondingObjectFromSource(attrib.gameObject) != null)
			...
			for (int i = 0; i < attrib.adPackages.Count; i++)
			{
				...
				if (!string.IsNullOrEmpty(downloadURlS[i]))
				...
				foreach (AdPackage.AdEvent adEvent in attrib.adPackages[i].adEvents)
				{
					...
					EditorGUI.BeginDisabledGroup(adEvent.type != AdPackage.AdEvent.Type.BANNER || attrib.adPackages[i].package == AdPackage.Package.CHARTBOOST);
					#if GOOGLE_MOBILE_ADS || UNITY_ADS
						adEvent.adPostion = (AdPackage.AdEvent.BannerPosition)EditorGUILayout.EnumPopup (adEvent.adPostion);
					#else
		...
		private void ShowAdmobInstructions()
		{
			...
			EditorGUILayout.Separator();
			EditorGUILayout.HelpBox("You have the option to download JDK/Android Studio", MessageType.None);
			...
			EditorGUILayout.HelpBox("5. Insert your Admob UnitIDs in the AdMob component below", MessageType.None);
			EditorGUILayout.HelpBox("6. (Optional) To test the Ads on your mobile device (Test Mode) : \n\n" +
				"- Get Test Ad Units from the link below\n" +
				"- Insert your Test Ad Units in the Admob component, then click on Apply button\n" +
		...
		{
			EditorGUILayout.HelpBox("1. Make sure Unity Advertisment package is installed from Window -> Package Manager", MessageType.None);

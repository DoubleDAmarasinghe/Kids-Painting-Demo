...
namespace IndieStudio.DrawingAndColoring.Utility
{
	...
	public class AdsManager : MonoBehaviour
	{
				...
				foreach (AdPackage.AdEvent adEvent in adPackage.adEvents)
				{
					if (adEvent.evt == evt)
					{
						if (adEvent.isEnabled)
						{
							...
							{
								UnityAdvertisment(adEvent,onShowAdsEvent);
							}
							eventFound = true;
						}
						break;
		...
		/// </summary>
		private void UnityAdvertisment(AdPackage.AdEvent adEvent, UnityEvent onShowAdsEvent)
		{
#if UNITY_ADS
						if (adEvent.type == AdPackage.AdEvent.Type.BANNER)
						{
							unityAds.LoadBanner(AdPackage.BannerPositionToUnityPosition(adEvent.adPostion));
						}
						else if (adEvent.type == AdPackage.AdEvent.Type.INTERSTITIAL)
						{
							unityAds.LoadInterstitialAd(onShowAdsEvent);
						}
						else if (adEvent.type == AdPackage.AdEvent.Type.RewardBasedVideo)
						{
							unityAds.LoadRewardedAd(onShowAdsEvent);
						}
#endif
		}
